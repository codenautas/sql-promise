...
sqlPromise.Query = function Query(sqlLib, internal, motor) {
    ...
    function fetchUniqueError(reject, throwerName) {
        var er = new Error("ERROR sql-promise."+throwerName);
        er.notUnique = true;
        reject(er);
    }
    this.fetchUniqueRow = function fetchUniqueRow() {
        var cont=0;
        function failWithError(reject) { fetchUniqueError(reject, 'fetchUniqueRow'); }
        var row=null;
        return Promises.make(function(resolve, reject){
            motor.fetchRowByRow(internal, {
                onRow:function(res) {
    ...
    this.fetchUniqueValue = function fetchUniqueValue() {
        var cont=0;
        function failWithError(reject) { fetchUniqueError(reject, 'fetchUniqueValue'); }
        var row=null;
        return Promises.make(function(resolve, reject){
            motor.fetchRowByRow(internal, {
                onRow:function(res) {
