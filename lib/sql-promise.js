...
sqlPromise.Query = function Query(sqlLib, internal, motor) {
    ...
    function createUniqueFetcher(name, result, setValue, resolve, reject) {
        ...
        fetcher.onRow = function(res) {
            ++result.count;
            if(result.count>result.maxRowCount) { failWithError(reject); }
            result.row = setValue(res);
        };
        fetcher.onEnd = function() {
            if(result.count<result.minRowCount) { failWithError(reject); }
    ...
    this.fetchUniqueRow = function fetchUniqueRow() {
        return Promises.make(function(resolve, reject){
            var fetcher=createUniqueFetcher('fetchUniqueRow', new Result(1,1),
    ...
    this.fetchUniqueValue = function fetchUniqueValue() {
        return Promises.make(function(resolve, reject){
            var fetcher=createUniqueFetcher('fetchUniqueValue', new Result(1, 1),
                function(res) { for(var nm in res) { return res[nm]; }
            }, resolve, reject);
            motor.fetchRowByRow(internal, {
                onRow:fetcher.onRow,
                onEnd:fetcher.onEnd,
    ...
    this.fetchOneRowIfExists = function fetchOneRowIfExists() {
        return Promises.make(function(resolve, reject){
            var fetcher=createUniqueFetcher('fetchOneRowIfExists', new Result(0, 1),
                function(res) { return res; }, resolve, reject);
            motor.fetchRowByRow(internal, {
                onRow:fetcher.onRow,
                onEnd:fetcher.onEnd,
                onError:reject
