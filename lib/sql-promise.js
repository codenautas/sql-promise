...
sqlPromise.Query = function Query(sqlLib, internal, motor) {
    ...
    this.fetchUniqueRow = function fetchUniqueRow() {
        return makeFetcherPromise('fetchUniqueRow', new Result(1,1), function(res) { return res; });
    }
    this.fetchUniqueValue = function fetchUniqueValue() {
        return makeFetcherPromise('fetchUniqueValue', new Result(1,1), function(res) { for(var nm in res) { return res[nm]; }});
    }
    this.fetchOneRowIfExists = function fetchOneRowIfExists() {
        return makeFetcherPromise('fetchOneRowIfExists', new Result(0,1), function(res) { return res; });
    }
};
